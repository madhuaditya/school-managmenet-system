...
import { useRouter } from 'expo-router'; // adjust import if needed based on your setup
import FontAwesome6 from '@expo/vector-icons/FontAwesome6';
import MaterialIcons from '@expo/vector-icons/MaterialIcons';
import Entypo from '@expo/vector-icons/Entypo';
...
// Removed dynamic dark/light mode for a unified flat theme.
const PALETTE = {
  primary: '#303841',
  accent: '#76ABAE',
  cta: '#FF5722',
...
const getStatusColors = (): Record<string, { bg: string; fg: string }> => ({
  completed: { bg: 'rgba(46, 125, 50, 0.1)', fg: PALETTE.success },
  completed_with_failures: { bg: 'rgba(249, 168, 37, 0.1)', fg: PALETTE.warning },
  failed: { bg: 'rgba(211, 47, 47, 0.1)', fg: PALETTE.error },
  processing: { bg: 'rgba(48, 56, 65, 0.1)', fg: PALETTE.primary },
  sent: { bg: 'rgba(46, 125, 50, 0.1)', fg: PALETTE.success },
  skipped: { bg: 'rgba(249, 168, 37, 0.1)', fg: PALETTE.warning },
});
    ...
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      <View style={styles.headerCard}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.subtitle}>Compose, preview, and track school-wide messages from one screen.</Text>
        </View>
        <Pressable
          onPress={refreshHistory}
          style={({ pressed }) => [
        ...
        >
          <MaterialIcons name="refresh" size={20} color={PALETTE.textHeading} />
        </Pressable>
      </View>

      {error ? <View style={[styles.banner, { backgroundColor: 'rgba(211, 47, 47, 0.1)' }]}><Text style={[styles.bannerText, { color: PALETTE.error }]}>{error}</Text></View> : null}
      {success ? <View style={[styles.banner, { backgroundColor: 'rgba(46, 125, 50, 0.1)' }]}><Text style={[styles.bannerText, { color: PALETTE.success }]}>{success}</Text></View> : null}

      <View style={styles.statsRow}>
        <StatCard label="Campaigns" value={summary.campaigns} accent="primary" />
        <StatCard label="Sent" value={summary.sent} accent="success" />
        <StatCard label="Failed" value={summary.failed} accent="danger" />
        <StatCard label="Skipped" value={summary.skipped} accent="warn" />
      </View>
      ...
      >
        {sectionState.create ? (
          <>
        <SectionLabel title="Delivery Channels" />
        <View style={styles.chipWrap}>
          {CHANNEL_OPTIONS.map((channel) => {
            const active = selectedChannels.includes(channel.value);
            return (
              <Pressable
                ...
                style={({ pressed }) => [
                  styles.channelCard,
                  {
                    backgroundColor: active ? 'rgba(48, 56, 65, 0.05)' : PALETTE.surface,
                    borderColor: active ? PALETTE.primary : PALETTE.border,
                    opacity: pressed ? 0.8 : 1,
                  },
                ]}
              >
                <View style={[styles.channelIcon, { backgroundColor: active ? PALETTE.primary : PALETTE.background, borderColor: active ? PALETTE.primary : PALETTE.border }]}>
                  <FontAwesome6 name={channel.icon} size={14} color={active ? '#fff' : PALETTE.textBody} />
                </View>
                <View style={styles.channelTextWrap}>
                  <Text style={styles.optionTitle}>{channel.label}</Text>
                  <Text style={styles.optionMeta}>{channel.description}</Text>
                </View>
              </Pressable>
            );
          })}
        ...
        <View style={styles.fieldGap}>
          <SectionLabel title="Target by Role" />
          <View style={styles.inlineChipRow}>
            {ROLE_OPTIONS.map((roleOption) => {
              const active = selectedRoles.includes(roleOption.value);
              return (
                ...
                >
                  <Text style={[styles.pillText, { color: active ? PALETTE.surface : PALETTE.textHeading }]}>{roleOption.label}</Text>
                </Pressable>
              );
            })}
          </View>
        ...
        <View style={styles.fieldGap}>
          <SectionLabel title="Target Classes" />
          <View style={styles.inlineChipRow}>
            {classOptions.map((classOption) => {
              const active = selectedClassIds.includes(classOption.id);
              return (
                ...
                >
                  <Text style={[styles.pillText, { color: active ? PALETTE.primary : PALETTE.textHeading }]}>{classOption.label}</Text>
                </Pressable>
              );
            })}
          </View>
          <Text style={styles.helperText}>Students for selected classes load automatically into the manual recipient list.</Text>
        </View>

        <View style={styles.fieldGap}>
          <SectionLabel title="Manual Recipients" />
          <RecipientGroup title="Admins" items={adminOptions} selectedIds={selectedAdminIds} onToggle={(id) => toggleValue(selectedAdminIds, setSelectedAdminIds, id)} />
          <RecipientGroup title="Teachers" items={teacherOptions} selectedIds={selectedTeacherIds} onToggle={(id) => toggleValue(selectedTeacherIds, setSelectedTeacherIds, id)} />
          <RecipientGroup title="Staff" items={staffOptions} selectedIds={selectedStaffIds} onToggle={(id) => toggleValue(selectedStaffIds, setSelectedStaffIds, id)} />
          <RecipientGroup title="Students" items={studentOptions} selectedIds={selectedStudentIds} onToggle={(id) => toggleValue(selectedStudentIds, setSelectedStudentIds, id)} />
        </View>
        ...
        <View style={styles.actionRow}>
          ...
          >
            {previewing ? <ActivityIndicator color={PALETTE.primary} /> : <><FontAwesome6 name="eye" size={14} color={PALETTE.primary} /><Text style={styles.actionTextSecondary}>Preview Recipients</Text></>}
          </Pressable>
      ...
      >
        {sectionState.preview ? (!preview ? (
          <EmptyState text="No preview generated yet." />
        ) : (
          <View style={styles.previewWrap}>
            <View style={styles.previewCount}>
              <Text style={styles.previewCountLabel}>Estimated recipients</Text>
              <Text style={styles.previewCountValue}>{preview.count}</Text>
            </View>
            <View style={styles.previewList}>
              {(preview.recipients || []).map((recipient: BroadcastPreviewRecipient) => (
                <View key={recipient._id} style={styles.previewItem}>
                  <Text style={styles.previewItemTitle}>{recipient.name || 'Unnamed user'}</Text>
                  <Text style={styles.previewItemMeta}>{recipient.role || 'unknown role'} | {recipient.email || recipient.phone || 'No contact'}</Text>
                </View>
              ))}
            </View>
          </View>
      ...
      >
        {sectionState.history ? (history.length === 0 ? (
          <EmptyState text="No broadcasts have been sent yet." />
        ) : (
          <View style={styles.historyList}>
            {history.map((item) => {
              const statusKey = String(item.status || 'unknown').toLowerCase();
              const colorSet = getStatusColors()[statusKey] || { bg: PALETTE.background, fg: PALETTE.textHeading };
              const summaryBlock = item.deliverySummary || emptySummary;
              return (
                <View key={item._id} style={styles.historyItem}>
                  <View style={styles.historyTopRow}>
                    <View style={styles.historyBody}>
                      <Text style={styles.historyTitle}>{item.title || 'Untitled broadcast'}</Text>
                      <Text style={styles.historyMessage}>{item.message || '-'}</Text>
                      <Text style={styles.historyMeta}>{formatDateTime(item.createdAt)} • {Array.isArray(item.channels) ? item.channels.join(', ') : '-'} • {item.recipientCount || 0} recipients</Text>
                    </View>
                    <View style={[styles.statusPill, { backgroundColor: colorSet.bg }]}>
                      <Text style={[styles.statusText, { color: colorSet.fg }]}>{statusKey.replaceAll('_', ' ')}</Text>
                    </View>
                  ...
                  <View style={styles.summaryRow}>
                    <SummaryBox label="Total" value={summaryBlock.total || 0} accent="primary" />
                    <SummaryBox label="Sent" value={summaryBlock.sent || 0} accent="success" />
                    <SummaryBox label="Failed" value={summaryBlock.failed || 0} accent="danger" />
                    <SummaryBox label="Skipped" value={summaryBlock.skipped || 0} accent="warn" />
                  </View>
      ...
      >
        {sectionState.delivery ? (loadingDeliveries ? (
          ...
          <EmptyState text="No delivery records found for this campaign." />
        ) : (
          <View style={styles.deliveryList}>
            {selectedBroadcast ? (
              <View style={styles.deliveryHeader}>
                <Text style={styles.deliveryHeaderTitle}>{selectedBroadcast.title || 'Broadcast'}</Text>
                <Text style={styles.deliveryHeaderMeta}>{formatDateTime(selectedBroadcast.createdAt)}</Text>
              </View>
            ) : null}

            {deliveries.map((delivery) => {
              const deliveryStatus = String(delivery.status || 'unknown').toLowerCase();
              const colorSet = getStatusColors()[deliveryStatus] || { bg: PALETTE.background, fg: PALETTE.textHeading };
              return (
                <View key={delivery._id} style={styles.deliveryCard}>
                  <View style={styles.deliveryCardTop}>
                    <View style={{ flex: 1 }}>
                      <Text style={styles.deliveryName}>{delivery.createdFor?.name || 'Unknown user'}</Text>
                      <Text style={styles.deliveryMeta}>{delivery.createdFor?.email || delivery.createdFor?.phone || '-'}</Text>
                    </View>
                    <View style={[styles.statusPill, { backgroundColor: colorSet.bg }]}>
                      <Text style={[styles.statusText, { color: colorSet.fg }]}>{deliveryStatus}</Text>
                    </View>
                  ...
                  <View style={styles.deliveryGrid}>
                    <DeliveryMeta label="Channel" value={delivery.channel || '-'} />
                    <DeliveryMeta label="Destination" value={delivery.destination || '-'} />
                    <DeliveryMeta label="Provider" value={delivery.provider || '-'} />
                    <DeliveryMeta label="Sent At" value={formatDateTime(delivery.sentAt || delivery.createdAt)} />
                  </View>

                  {delivery.errorMessage ? <Text style={styles.errorText}>{delivery.errorMessage}</Text> : null}
                </View>
              );
            })}
          </View>
      ...
      <Pressable onPress={onToggle} style={styles.sectionHeader}>
        ...
        <View style={styles.sectionHeaderActions}>
          <View style={styles.sectionBadge}>
            <Entypo name="megaphone" size={16} color={PALETTE.textBody}/>
          </View>
          <MaterialIcons name={expanded ? 'expand-less' : 'expand-more'} size={22} color={PALETTE.textBody} />
        </View>
      </Pressable>
      {expanded ? children : null}
    </View>
...
function StatCard({ label, value, accent }: { label: string; value: number; accent?: 'primary' | 'success' | 'danger' | 'warn'; }) {
  const accentColor = accent === 'success' ? PALETTE.success : accent === 'danger' ? PALETTE.error : accent === 'warn' ? PALETTE.warning : PALETTE.primary;
  const accentBg = accent === 'success' ? 'rgba(46, 125, 50, 0.1)' : accent === 'danger' ? 'rgba(211, 47, 47, 0.1)' : accent === 'warn' ? 'rgba(249, 168, 37, 0.1)' : 'rgba(48, 56, 65, 0.1)';
  return (
...
function SummaryBox({ label, value, accent }: { label: string; value: number; accent?: 'primary' | 'success' | 'danger' | 'warn'; }) {
  const accentColor = accent === 'success' ? PALETTE.success : accent === 'danger' ? PALETTE.error : accent === 'warn' ? PALETTE.warning : PALETTE.textHeading;
  return (
...
function RecipientGroup({
  ...
  return (
    <View style={styles.recipientGroup}>
      <Text style={styles.recipientGroupTitle}>{title}</Text>
      {items.length === 0 ? (
        <Text style={styles.helperText}>No records found.</Text>
      ) : (
        <View style={styles.inlineChipRow}>
          {items.map((item) => {
            const active = selectedIds.includes(item.id);
            return (
              ...
              >
                <Text style={[styles.personTitle, { color: active ? PALETTE.primary : PALETTE.textHeading }]} numberOfLines={1}>
                  {item.label}
                </Text>
                {item.meta ? <Text style={styles.personMeta} numberOfLines={1}>{item.meta}</Text> : null}
              </Pressable>
            );
          })}
        </View>
...
function DeliveryMeta({ label, value }: { label: string; value: string; }) {
  return (
    <View style={styles.deliveryMetaBox}>
      <Text style={styles.deliveryMetaLabel}>{label}</Text>
      <Text style={styles.deliveryMetaValue}>{value}</Text>
    </View>
  );
}
