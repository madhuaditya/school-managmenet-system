...
export default function TeachersScreen() {
  ...
  // Included and styled in case it is used instead of AttendanceRoster in some views
  const renderTeacherCard = ({ item }: { item: TeacherRecord }) => {
    const subjectLabel = (item.teachSubjects || [])
      .map((subject) => (subject.code ? `${subject.name} (${subject.code})` : subject.name))
      .filter(Boolean)
    ...
    return (
      <View style={styles.card}>
        ...
        <Text style={styles.meta}>Class Teacher: {classLabel}</Text>
        <Text style={styles.meta}>Subjects: {subjectLabel || 'N/A'}</Text>
...
const styles = StyleSheet.create({
  ...
  /* TEACHER CARD (If used outside of Roster) */
  card: {
    borderWidth: 1,
    borderColor: PALETTE.border,
    backgroundColor: PALETTE.surface,
