...
import React, { useCallback, useMemo, useRef, useState } from 'react';
import { FlatList, View, StyleSheet, Dimensions, Pressable, Modal, Text } from 'react-native';
import AttendanceRow from '@/components/AttendanceRowStudent';
import { ClassItem } from '@/app/(tabs)/students';
...
function SummaryCard({
  ...
  return (
    <View style={styles.summaryCard}>
      ...
      {selectedClassData && (
        <View style={styles.classInfo}>
          <Text style={styles.className}>{selectedClassData.name}</Text>
          <Text style={styles.classSection}>{selectedClassData.section}</Text>
        </View>
      )}
      <Text style={styles.summaryText}>Total students: {total}</Text>
      <View style={styles.summaryGrid}>
        <SummaryStat label="Present" value={counts.present} tone="present" />
        <SummaryStat label="Absent" value={counts.absent} tone="absent" />
        <SummaryStat label="Leave" value={counts.leave} tone="leave" />
...
export default function AttendanceLayoutStudent({ roster, updateStatus, mode = 'list', onSubmitBulk, submitting, classes, selectedClassId, pendingCount }: Props) {
  ...
  const cardHeight = Math.max(380, Math.min(screenHeight - 120, 520));
  const horizontalMargin = 12;
  const cardWidth = Math.min(screenWidth - 10, 1200);
  const itemWidth = cardWidth + horizontalMargin * 2;
  const [showSubmitPopup, setShowSubmitPopup] = useState(false);
    ...
    <View style={styles.container}>
      ...
      {mode === 'card' ? (
          ...
          <Modal visible={showSubmitPopup} transparent animationType="fade" onRequestClose={() => setShowSubmitPopup(false)}>
            <View style={styles.modalOverlay}>
              <View style={styles.modalCard}>
                ...
                <Text style={styles.modalMessage}>Your attendance has been submitted successfully.</Text>
                <Pressable onPress={() => setShowSubmitPopup(false)} style={({ pressed }) => [styles.modalButton, pressed && styles.modalButtonPressed]}>
        ...
        <FlatList
          ...
          updateCellsBatchingPeriod={50}
          ItemSeparatorComponent={() => <View style={styles.separator} />}
          ListEmptyComponent={<Text style={styles.empty}>No students found in this class.</Text>}
        />
      )}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  quickNavSideButton: {
    ...
    width: 44,
    height: 44,
    borderRadius: 4, // Clean flat look instead of circular
    alignItems: 'center',
    justifyContent: 'center',
    ...
    zIndex: 999,
    elevation: 2, // Very slight shadow
  },
  quickNavLeft: { left: 8 },
  quickNavRight: { right: 8 },
  quickNavPressed: {
  ...
  cardPage: {
    width: '100%',
    padding: 4,
    paddingRight: 30, // Extra space for shadow on right edge
  },
  ...
  modalOverlay: {
    flex: 1,
    backgroundColor: 'rgba(48, 56, 65, 0.6)', // Primary color with opacity
    alignItems: 'center',
    justifyContent: 'center',
    padding: 20,
  },
